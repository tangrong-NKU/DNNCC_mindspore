...
class MyWithLossCell(nn.Cell):
    ...
    def construct(self, batch_y,r_att, p_att, r_fun, p_fun, idx):
        res = self.backbone(r_att, p_att, r_fun, p_fun, idx)
        loss = self.loss_func(res,batch_y)
        return loss
...
def model_train(BATCH_SIZE, ATT_SIZE, FUN_SIZE):
    ...
    for f in range(1):
        ...
        for data in dataloader.create_dict_iterator():
            ...
            e_r, e_p,output = model(xr_m_tensor, xp_m_tensor, xr_c_tensor, xp_c_tensor, batch_id)

            loss = loss_func1(output,batch_y)
            print(loss)
