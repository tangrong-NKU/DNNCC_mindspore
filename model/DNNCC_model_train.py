...
class MLP_CNN(nn.Cell):
    ...
    def construct(self, r_att, p_att, r_fun, p_fun, idx):
        ...
        fun = zero_cat((r_fun, p_fun))
        e_fun = self.encoder_fun(fun)
        e = one_cat((e_att, e_fun))
        ...
        for i in range(len(idx)):
            ...
            x_cnn.append(zero_cat((e_r[r_no, :], e_p[p_no, :])))
        print(x_cnn[0])
        print(type(x_cnn))
        print(len(x_cnn))
        x_cnn = x_cnn.asnumpy().tolist()
        x_cnn = zero_cat(tuple(x_cnn))
...
def model_train(BATCH_SIZE, ATT_SIZE, FUN_SIZE):
    ...
    for f in range(1):
        ...
        dataloader = dataloader.batch(256)
        ...
        for epoch in range(EPOCH):
            for step, data in enumerate(dataloader):
                ...
                modelx = ms.Model(model)
                modelx.train(epoch=100,train_dataset=(xr_m_tensor, xp_m_tensor, xr_c_tensor, xp_c_tensor, batch_idx))
